...
import com.vrhub.data.FilePaths
import com.vrhub.data.GameNotFoundException
import com.vrhub.data.InsufficientStorageException
import com.vrhub.data.InstallStatus
import com.vrhub.data.MirrorNotFoundException
import com.vrhub.data.NetworkModule
import com.vrhub.data.NoDownloadableFilesException
import com.vrhub.data.NonRetryableDownloadException
import com.vrhub.data.toData
...
class DownloadWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        // Get server config from input data (set by MainRepository before enqueueing)
        val baseUri = inputData.getString(KEY_BASE_URI)
            ?: throw IllegalStateException("Missing baseUri in download work input")
        val password = inputData.getString(KEY_PASSWORD)
            ?: throw IllegalStateException("Missing password in download work input")

        Log.i(TAG, "Starting download work for: $releaseName (attempt: $runAttemptCount)")

        return try {
            setForeground(getForegroundInfo())
            // Decode password from Base64 as done in MainRepository
            decodedPassword = try {
                if (password.isNotBlank()) {
                    String(android.util.Base64.decode(password, android.util.Base64.NO_WRAP))
                } else null
            } catch (e: Exception) {
                Log.w(TAG, "Failed to decode password", e)
                null
            }
            executeDownload(releaseName, isDownloadOnly, keepApk, baseUri)
